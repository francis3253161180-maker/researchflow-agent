...
- `pytest` 覆盖 55 项单元、API 与 MCP 端到端测试，包括上传/删除、页码元数据、结构化引用校验、会话/turn 恢复、per-run thinking mode、只使用已验证 turn 的会话感知 Query Rewrite、SSE 节点状态与最终结果、首轮模型标题、异常脱敏、轨迹持久化以及 stdio 工具发现/调用。
