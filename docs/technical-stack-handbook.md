...
**61项测试说明什么？** 说明当前明确行为有自动回归保护；其中包含会话恢复、per-run thinking mode、首轮模型标题、可信短期记忆 Query Rewrite、PDF/DOCX 结构元数据、SSE 节点状态、按 Verify 优先级区分的检索/生成修复、MCP 工具与 stdio 协议调用，不说明生产高并发、真实性能或全部边界已覆盖。
